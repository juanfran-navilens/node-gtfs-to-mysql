...
const markRealtimeDataStale = (config, log) => {
  ...
  db.prepare(`UPDATE vehicle_positions SET isUpdated=0`);
  db.prepare(`UPDATE trip_updates SET isUpdated=0`);
  db.prepare(`UPDATE stop_times_updates SET isUpdated=0`);
  db.prepare(`UPDATE service_alerts SET isUpdated=0`);
  db.prepare(`UPDATE service_alert_targets SET isUpdated=0`);
...
const cleanStaleRealtimeData = (config, log) => {
  ...
  db.prepare(`DELETE FROM vehicle_positions WHERE isUpdated=0`);
  db.prepare(`DELETE FROM trip_updates WHERE isUpdated=0`);
  db.prepare(`DELETE FROM stop_times_updates WHERE isUpdated=0`);
  db.prepare(`DELETE FROM service_alerts WHERE isUpdated=0`);
  db.prepare(`DELETE FROM service_alert_targets WHERE isUpdated=0`);
...
const createTables = (db) => {
  for (const model of models) {
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS ${model.filenameBase} (${columns.join(', ')});`);
